...
| 2025-10-28 | **Rough Proof of UCB Bound** | Came up with rough proof that $\lvert f^*(\bm{x}) - f_{\text{ViT}}(\bm{x}; \bm{\theta}_{\text{ViT}}, \bm{\theta}_{\text{LoRA}}) \rvert \le \alpha \,\| \frac{\bm{g}_{\text{ViT}}(\bm{x}; \bm{\theta}_{\text{LoRA}})}{\sqrt{d_{\text{LoRA}}}} \|_{\bm{A}_t^{-1}} + \beta$ |
